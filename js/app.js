...
(function () {
  ...
  /* ——————————————————— ЗАЯВКИ ———————————————————
     Заявки уходят в Telegram-канал через посредника на Google Apps Script
     (tools/telegram-proxy). Токен бота хранится там, на сайте его нет.
     Сюда вставляется только адрес развёрнутого скрипта.
     Пока адрес пустой, формы работают в демо-режиме — заявка пишется в консоль. */
  var LEADS_ENDPOINT = '';   // https://script.google.com/macros/s/…/exec
  ...
  function sendLead(data, file) {
    ...
    ['topic', 'name', 'phone', 'category', 'note', 'page'].forEach(function (k) { if (data[k]) body.append(k, data[k]); });
    var ready = file
      ? fileToBase64(file).then(function (b64) {
          body.append('file_name', file.name);
          body.append('file_type', file.type || 'application/octet-stream');
          body.append('file_b64', b64);
        })
      : Promise.resolve();
    // URLSearchParams — «простой» запрос, без предварительного CORS-запроса;
    // Apps Script отвечает через редирект на googleusercontent.com, fetch проходит его сам
    return ready
      .then(function () { return fetch(LEADS_ENDPOINT, { method: 'POST', body: body, redirect: 'follow' }); })
      .then(function (r) { return r.json(); })
      .then(function (j) { if (!j.ok) throw new Error(j.error || 'Посредник вернул ошибку'); return j; });
  }
