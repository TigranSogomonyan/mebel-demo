...
(function () {
  ...
  /* ——————————————————— TELEGRAM ———————————————————
     Заявки уходят в канал через бота. Токен виден в коде страницы:
     если в канал начнут писать посторонние, перевыпустите токен
     в @BotFather (команда /revoke) и замените его здесь.
     Пока поля пустые, формы работают в демо-режиме — заявка пишется в консоль. */
  var TELEGRAM = {
    token: '',    // 123456789:AAE…  — выдаёт @BotFather
    chatId: ''    // @имя_канала для публичного или -100… для закрытого
  };
  ...
  function leadText(data) {
    var rows = [['Имя', data.name], ['Телефон', data.phone], ['Интересует', data.category], ['Комментарий', data.note]]
      .filter(function (r) { return r[1] && String(r[1]).trim(); })
      .map(function (r) { return '<b>' + r[0] + ':</b> ' + esc(String(r[1]).trim()); });
    var when = new Date().toLocaleString('ru-RU', { timeZone: 'Europe/Moscow', day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' });
    return '<b>Новая заявка · FAM</b>\n' + esc(data.topic) + '\n\n' + rows.join('\n') +
      '\n\n<i>' + when + ' МСК</i>';
  }

  function tgCall(method, body) {
    return fetch('https://api.telegram.org/bot' + TELEGRAM.token + '/' + method, { method: 'POST', body: body })
      .then(function (r) { return r.json(); })
      .then(function (j) { if (!j.ok) throw new Error(j.description || 'Telegram API'); return j; });
  }

  function sendLead(data, file) {
    // URLSearchParams и FormData не требуют предварительного CORS-запроса
    var msg = new URLSearchParams({ chat_id: TELEGRAM.chatId, text: leadText(data), parse_mode: 'HTML', disable_web_page_preview: 'true' });
    return tgCall('sendMessage', msg).then(function () {
      if (!file) return;
      var fd = new FormData();
      fd.append('chat_id', TELEGRAM.chatId);
      fd.append('document', file, file.name);
      fd.append('caption', 'План к заявке: ' + (data.name || '') + ', ' + (data.phone || ''));
      return tgCall('sendDocument', fd);
    });
  }
  ...
  function initForms() {
    ...
    $$('form').forEach(function (form) {
      ...
      form.addEventListener('submit', function (e) {
        ...
        // тема из кнопки, открывшей попап (например, конкретный проект), важнее общей темы формы
        var inPopup = !!form.closest('.popup');
        var data = {
          topic: (inPopup && pendingTopic) || form.dataset.formTopic || 'Заявка с сайта',
          page: location.href
        };
        ...
        var done = function () {
          ...
          if (t) t.textContent = data.name
            ? data.name + ', спасибо! Заявку получили — свяжемся с вами в течение того же дня.'
            : 'Спасибо! Заявку получили — свяжемся с вами в течение того же дня.';
          Popup.open('thanks');
        };
        var fail = function (msg) {
          if (btn) { btn.disabled = false; btn.textContent = label; }
          formStatus(form, msg || 'Не получилось отправить заявку. Позвоните нам: +7 (966) 194-03-33 — или напишите в Telegram.');
        };

        // ловушка для ботов: поле скрыто от людей, спам-скрипты его заполняют
        if (data.website) { setTimeout(done, 600); return; }
        delete data.website;

        if (file && file.size > 20 * 1024 * 1024) { fail('Файл больше 20 МБ. Пришлите план поменьше или отправьте его в Telegram.'); return; }

        var last = 0;
        try { last = +localStorage.getItem('fam-lead-sent') || 0; } catch (err) {}
        if (Date.now() - last < 30000) { fail('Заявка уже отправлена. Если нужно что-то добавить, подождите полминуты.'); return; }

        if (!TELEGRAM.token || !TELEGRAM.chatId) {
          console.info('Демо-режим, Telegram не настроен. Заявка:', data, file);
          setTimeout(done, 600);
          return;
        }

        sendLead(data, file).then(function () {
          try { localStorage.setItem('fam-lead-sent', String(Date.now())); } catch (err) {}
          done();
        }).catch(function (err) {
